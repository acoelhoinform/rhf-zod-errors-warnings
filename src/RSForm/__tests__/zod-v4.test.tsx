...
describe("zodResolver", () => {
  it.skip("should return values from RSZodResolver when validation pass & raw=true", async () => {
  ...
  it.skip("should return parsed values from RSZodResolver with `mode: sync` when validation pass", async () => {
  ...
  it.skip("should return a single error from RSZodResolver with `mode: sync` when validation fails", async () => {
    const parseSpy = vi.spyOn(schema, "parse");
    const parseAsyncSpy = vi.spyOn(schema, "parseAsync");
  ...
  it("should correctly infer the output type from a zod schema when a different input type is specified", () => {
    ...
    expectTypeOf(resolver).toEqualTypeOf<
      RSResolver<{ id: number }, any, { id: string }>
    >();
  });
