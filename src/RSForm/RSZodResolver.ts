// Helper to check if an issue has params (custom Zod issues)
function hasParams(issue: any): issue is { params: any } {
  return typeof issue === "object" && issue !== null && "params" in issue;
}
import * as z3 from 'zod/v3';
import * as z4 from 'zod/v4/core';
import { RSFieldError } from './Errors';
import { appendErrors, FieldValues, ResolverSuccess  } from 'react-hook-form';
import { RSResolver, RSResolverError } from './Resolver';
...
function parseZod3Issues(
  ...
  const errors: Record<string, RSFieldError> = {};
  for (; zodErrors.length; ) {
    ...
    if (!errors[_path]) {
      if ('unionErrors' in error) {
        ...
        errors[_path] = { message, type: code, ...(hasParams(error) ? { params: error.params } : {}) };
      }
    }
    ...
    if (!errors[_path]) {
      if (error.code === 'invalid_union') {
        ...
        errors[_path] = {
          message: unionError.message,
          type: unionError.code,
          ...(hasParams(unionError) ? { params: unionError.params } : {})
        };
      } else {
        errors[_path] = {
          message,
          type: code,
          ...(hasParams(error) ? { params: error.params } : {})
        };
      }
    }
...
>(
  ...
  resolverOptions?: NonRawResolverOptions,
): RSResolver<z4.input<T>, Context, z4.output<T>>;
export function RSZodResolver<
  Input extends FieldValues,
  Context,
  Output,
...
>(
  ...
  resolverOptions: RawResolverOptions,
): RSResolver<z4.input<T>, Context, z4.input<T>>;
/**
...
): RSResolver<Input, Context, Output | Input> {
    ...
    return async (values: Input, _, options) => {
      try {
        ...
        if (isZod3Error(error)) {
          ...
          } satisfies RSResolverError<Input>;
        }
        throw error;
      }
    };
  } else if (isZod4Schema(schema)) {
    return async (values: Input, _, options) => {
      try {
        const parseFn =
